...
| --------------- | ------------------------- | ----------------------------------------------------------- |
| Patient         | `riyaz@stride.clinic`     | Large-type home plan, start a movement, save repetitions    |
...
| **Implementation-independent** | Describes _what_ the module must do, not _how_ it is coded                         |
...
| M10 Video consult   | `/video/room`, `/video/ws` | Web `VideoRoom`, mobile `ConsultScreen` | _(none — ephemeral signaling)_           |
...
Reset/seed the local SQLite demo (idempotent without `--reset`): `python -m app.seed --reset` from `apps/api`. Development/demo credentials only — see §2.
