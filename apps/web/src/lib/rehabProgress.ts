...
export function planAdherence(
  ...
  if (!prescribed) {
    return {
      ...
      prescribed: 0,
      percent: null,
      label: "No exercises prescribed yet",
    };
  }
...
export function todayAdherence(
  plan: PlanLike,
  sessions: SessionLike[],
): AdherenceSummary {
...
export function confirmedObservationTrends(
...
export function latestPatientReported(
  sessions: SessionLike[],
): SessionLike | null {
  const withNotes = sessions
    .filter(
      (s) =>
        s.status === "completed" &&
        (s.patient_notes || s.reported_repetitions != null),
    )
    .slice()
    .sort((a, b) =>
